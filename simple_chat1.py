...
from flask import Flask, request, jsonify  # Toegevoegd voor HTTP API
...
def handle_test_internet_wget():
    try:
        # Maak unieke bestandsnamen met timestamp
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        test_file = f"{WGET_LOG_DIR}/{TEST_FILE_PREFIX}_{timestamp}.html"
        log_file = f"{WGET_LOG_DIR}/wget_log_{timestamp}.txt"

        # Controleer en maak log-directory
        ...
        # Stel proxy-omgeving in
        env = os.environ.copy()
        env["http_proxy"] = "http://localhost:3128"

        # Voer wget uit met logbestand
        subprocess.run(
            ["wget", "--debug", "--output-file", log_file, "-O", test_file, "http://example.com"],
            timeout=10,
            env=env,
            check=True
        )

        # Controleer of test_file bestaat en lees inhoud
        ...
        # Log succes naar chatlog
        save_chat(f"Wget test: Success (Bestand: {test_file}, Log: {log_file})")
